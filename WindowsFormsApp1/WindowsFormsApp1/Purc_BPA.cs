...
{
    public partial class Purc_BPA : UserControl
    {
        public Purc_BPA()
        {
            InitializeComponent();
        ...
        public int indexRow;
        Purc_EditContract eCon = new Purc_EditContract();
        ...
        private void update()
        {
            ...
            string sqlStr = "Select ContractID, StaffID, SigningDate, supID, EffectiveDate, ExpirationDate from Contract WHERE Type = 'BPA' ";
            OleDbConnection conn = new OleDbConnection(connStr);
            conn.Open();
            OleDbDataAdapter oleDbDataAdapter = new OleDbDataAdapter(sqlStr, conn);
            oleDbDataAdapter.Fill(ds);
            dgvContract.DataSource = ds.Tables[0];
        }
        ...
        {
            Purc_NewContract newC = new Purc_NewContract();
            newC.ShowDialog();
            update();
        }
        ...
        {
            if (dgvContract.CurrentCell != null && dgvContract.CurrentRow.Index < dgvContract.Rows.Count)
            {
                try
                {
                    indexRow = dgvContract.CurrentCell.RowIndex;
                    DataGridViewRow row = dgvContract.Rows[indexRow];
                    string connStr = "Provider = Microsoft.ACE.OLEDB.12.0; " + "Data Source=Database.accdb";
