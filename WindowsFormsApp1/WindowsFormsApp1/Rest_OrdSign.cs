...
namespace WindowsFormsApp1
{
    ...
    public partial class Rest_OrdSign : UserControl
    {
        ...
        private void Rest_OrdSign_Load(object sender, EventArgs e)
        {
            ...
            while (delireader.Read())
            {
                ...
                flpordsign.Controls.Add(deliOrdUC);
            }
            conn.Close();
        }
        ...
        {
            string sqlreq = "SELECT * FROM [Delivery] WHERE Status ='In Transit'";
            OleDbCommand ReqQuery = new OleDbCommand(sqlreq, conn);
            return ReqQuery.ExecuteReader();
        }
        private void UpdateDeliveryStatus(String DeliID, OleDbConnection conn)
        {
            string updatereq = "UPDATE [Delivery] SET Status ='Delivered' WHERE [DeliID]=@DID";
        ...
        private void Onclick(object sender, EventArgs e)
        {
            ...
            UpdateOrderStatus(DeliID, conn);

            flpordsign.Controls.Clear();
            OleDbDataReader delireader = GetDeliOrd(conn);
            while (delireader.Read())
            {
                UC_DeliOrd deliOrdUC = new UC_DeliOrd();
        ...
        private void UpdateOrderStatus(String Did, OleDbConnection conn)
        {
            string updatereq = "UPDATE [Order] SET Status = 'Delivered' WHERE OrdID = (SELECT OrderID FROM Delivery WHERE Deliid =@did) AND Status = 'Processing'";
            OleDbCommand cmdupdatereq = new OleDbCommand(updatereq, conn);
            cmdupdatereq.Parameters.AddWithValue("@DID", Did);
            cmdupdatereq.ExecuteNonQuery();
        }
    }
}
